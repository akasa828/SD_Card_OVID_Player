...
## v1.3.2-beta.1 OVID 转换器界面调整（开发中）

### OVID Converter

- 整理“更多设置”的响应式栅格：目录读取与覆盖选项、线程数与快速视频模式分别对齐为两列，窄窗口自动切换为单列。
- 将“重新载入预览”固定到内容左侧，避免下拉框、开关和按钮各自居中造成视觉轴线混乱。

## v1.3.1 OVID 转换器可用性与可靠性更新（2026-08-27）

### OVID Converter

- 根据实际 Windows 桌面录屏调整响应式参数布局：预览与参数卡并排时，画面尺寸和黑白处理改为纵向分组，避免默认窗口下范围文字、缩放选项和 Floyd 抖动被挤压；参数卡独占足够宽度时仍保留双栏。
- 空闲状态的底部任务栏不再重复显示“准备就绪”，只有转换、停止或完成结果需要补充信息时才展开第二行，增加小窗口中的有效内容高度。
- 未导入素材时收起空预览、路径、任务列表和底部转换栏，首页集中显示素材导入入口；添加素材使用主题主按钮，图片目录保留独立入口，导入后恢复完整编辑布局。
- 增加“先设置转换参数”，无需导入文件即可调整参数、应用或保存预设；收起再展开保留当前草稿，后续添加的任务继承这些参数，批量任务仍各自独立保存。
- 清空最后一个任务后恢复初始页并清除旧进度；收起参数后导入遇到无效数字，会自动展开并提示。恢复的任务无需重新导入即可显示，隐藏区域和其他页面不接收无效的局部刷新。
- 弹窗按钮改为只关闭对应窗口，后台新出现的状态提示不再被误关闭；取消、Esc 关闭后的迟到确认和重复点击不会再次执行预设保存、删除、阈值应用或裁剪。
- 预设、阈值、裁剪和退出确认会检查当前最上层弹窗，避免被遮挡窗口的迟到事件修改参数或触发退出；保留原有关闭回调与页面快捷键隔离。
- 保存预设的名称、参数和目标设备由各自弹窗独立持有；支持回车保存，空名称或内置预设重名在输入框下提示，可直接修正，覆盖确认仍需明确同意。
- 调整宽高、FPS、缩放、补边或应用预设后，预览保留当前时间点，不再跳回首帧；图片目录修改 FPS 时保留同一张图片，重新载入预览也保留位置，“回到首帧”仍明确回到导出范围起点。
- 预览位置按新 FPS 对齐到有效帧，并限制在裁剪和跳帧后的范围内；定位仅修改预览解码参数，不改写任务导出范围，预计大小仍按完整任务计算。
- 连续参数修改使用最后显示画面的位置快照，旧重载不会覆盖新请求；切换素材、清空任务或预览失败后清除旧位置。图片目录的 FPS 改变后同步更新时间轴长度。
- 参数重载超过 150 ms 时在预览下方提示“正在更新预览”，等待期间保留旧画面；完成、失败或被新请求取代后取消提示，避免迟到文字覆盖新结果。
- 裁剪、预设、错误详情和像素查看等弹窗打开时，屏蔽背后页面的播放、逐帧、文件导入与转换快捷键；普通状态提示不影响快捷键，关闭弹窗后恢复操作。
- `Ctrl+O` 按当前页面打开转换素材或 OVID 文件，`Ctrl+Enter` 仅在转换页生效，设置与关于页面不再误触发转换；播放器打开按钮补充快捷键提示。
- 修复退出确认框被 Esc 或外部点击关闭后仍保留确认标记的问题，关闭后可再次请求退出；统一弹窗管理保留原有关闭回调，不访问 Flet 私有弹窗栈。
- 设备屏幕下方即时显示输出尺寸是否匹配、可居中显示或已超屏，并按页主序计算每帧字节数；自定义设备明确提示尚未检查屏幕尺寸。
- 增加“使用设备尺寸”按钮，仅明确点击后同步输出宽高，保留 FPS、裁剪与其他任务参数；尺寸已经一致、自定义设备或任务锁定时禁用该操作。
- 尺寸与目标设备的迟到事件不会改写已冻结任务，正在转换时界面会恢复实际任务参数；尺寸更新在等待预览期间再次核对锁定状态，避免晚到的预览重载。
